...
public partial class EmployeeForm : Form
{
    ...
    private void RefreshRequestsView()
    {
        ...
        if (allRequests is not null && allRequests.Count > 0)
        {
    ...
    private void HandleDeleteResource(int resourceID)
    {
        ...
        var selectedResource = api.GetResourceById(resourceID);
        ...
        if (result == DialogResult.Yes)
        {
            ...
            if (approvedRequests is not null && approvedRequests.Any(request => request.ResourceID == resourceID))
            {
            ...
            if (requestsToDelete is not null && requestsToDelete.Count > 0)
            {
                // Display a confirmation message
    ...
    private void HandleReturn(int copyID)
    {
        ...
        var approvedRequests = api.GetBorrowRequests(Status.Approved);
        var approvedRequest = approvedRequests is null? null : approvedRequests.FirstOrDefault(request => request.CopyID == copyID);
        ...
        if (currentRequest != null)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                else
                {
                    ...
                    _ = api.UpdateBorrowRequestStatusToApproved(requestID, copyID, dueDate);
                    RefreshExtensionsRequestsView();
                }
            }

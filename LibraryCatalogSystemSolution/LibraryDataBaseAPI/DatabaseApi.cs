...
public class DatabaseApi
{
    ...
    public DatabaseApi(string provider, string connectionString)
    {
        this.connector = new(provider, connectionString);
    }

    public const string EMPLOYEE_KEY = "EmployeeKey";
    ...
    /// <summary>
    /// Retrieves resource with given ID.
    /// </summary>
    /// <returns>
    /// A Resource objects if success, otherwise null.
    /// </returns>
    public Resource? GetResourceById(int id)
    {
        var reader = connector.GetFromDataBase($"SELECT * FROM Resources WHERE ResourceID = {id}");
        ...
        if(res.Count != 1) 
            throw new Exception("Multiple resources with equal ID!");
